...
left join 
(select point,date,coalesce(sum(out),NULL) outcome 
from outcome 
group by point,date) o on o.date=a.date and o.point=a.point
left join 
(select point,date,coalesce(sum(inc),NULL) income 
from income 
group by point,date) i on i.date=a.date and i.point=a.point
